...
import athena.coder.ai.assistant.agent.test.TestReviewAgent;
...
public class TestReviewNode extends AbstractAgentNode {
    ...
    protected Map<String, Object> doApply(WorkflowState state, NodeContext ctx) throws Exception {
        ...
        String changedFiles = requireUpstream(state.getStringValue(CHANGED_FILES),
                "changedFiles 为空，无测试变更可审查（需要补测环节先完成）");
        ...
        String testResult = requireUpstream(state.getStringValue(TEST_RESULT),
                "testResult 为空，缺少测试执行证据（需要测试执行环节先完成）");
        String acceptanceCriteria = state.getStringValue(ACCEPTANCE_CRITERIA);

        warnIfBlank(changedDiffRef, "changedDiffRef 为空，将无法查看具体的测试变更 diff");
        ...
        TestReviewAgent assistant = newChatAssistant(ctx.modelType(), TestReviewAgent.class);
        ...
        logInfo(String.format("TestReviewNode 审查完成: verdict=%s, result长度=%d", verdict, reviewResultJson.length()));
        ...
        String reviewMsg = switch (verdict) {
            case "APPROVED" -> "测试质量审查通过";
            case "APPROVED_WITH_NOTES" -> "测试质量审查通过（有建议）";
            case "REQUEST_CHANGES" -> "测试质量审查打回，需要修改";
            case "BLOCKED" -> "测试质量审查严重阻塞，需人工介入";
        ...
        return switch (verdict) {
            ...
            case "REQUEST_CHANGES" -> {
                if (loopCount >= MAX_REVIEW_LOOPS) {
                    ErrorLogger.warn("TestReviewNode", "审查打回已达上限(" + MAX_REVIEW_LOOPS + "次)，熔断并路由到 SUMMARIZER 收尾");
                    yield SUMMARIZER.name();
                }
                ErrorLogger.warn("TestReviewNode", "审查未通过(REQUEST_CHANGES，第" + loopCount + "轮)，路由到 CODER 进行修改");
                yield CODER.name();
            }
            case "BLOCKED" -> {
                ErrorLogger.warn("TestReviewNode", "审查严重阻塞(BLOCKED)，路由到 SUMMARIZER 输出收尾报告并提示人工介入");
                yield SUMMARIZER.name();
            }
            default -> {
