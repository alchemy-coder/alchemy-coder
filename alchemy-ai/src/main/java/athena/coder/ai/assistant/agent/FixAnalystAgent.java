...
public interface FixAnalystAgent {
    ...
    @SystemMessage("# Fix Analyst Agent - 调试分析员\n"
            ...
            2. **只分析不修改** - 可读代码、跑诊断、搜索模式，绝不修改文件
            3. **证据驱动** - 必须先调工具获取实际数据，禁止凭猜测下结论
            4. **历史反思** - 分析前逐条核对 previousFixes，禁止重复已尝试过且失败的策略（即使表述不同）
    ...
    DebuggerResult analyze(@MemoryId long memoryId,
                           @UserMessage("{{debugRequest}}" + ANALYST_EVIDENCE + PROJECT_FACTS_BLOCK)
