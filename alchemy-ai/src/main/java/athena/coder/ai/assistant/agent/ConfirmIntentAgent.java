 ...
 * <p>
 * 职责：判定用户对执行计划的回复是确认 / 提修改意见 / 追问答疑 / 拒绝取消；REVISE 时同时提炼结构化修订指令。
 * <p>
 * 工具集：无（纯LLM推理）
 */
public interface ConfirmIntentAgent {
    ...
    @SystemMessage(JSON_OUTPUT_RULE + """
            {
              "intent": "CONFIRM或REVISE或CLARIFY或REJECT",
              "revise": {
            ...
            REVISE  — 用户提出修改意见、部分认可但要求调整、或要求换个方向（仍想继续，只是要改）
            CLARIFY — 用户仅提问 / 有疑问、未表态、未提出任何修改指令（如“任务3的风险怎么规避”）
            REJECT  — 用户明确取消/否定整体且不打算继续（如“算了不做了”、“取消吧”、“别继续了”）

            CLARIFY 与 REVISE 的边界（关键）：
            - 纯疑问句、只求解释、不要求改动 → CLARIFY（例：“任务3为什么要用消息队列”、“这个方案能支持并发吗”）
            - 提问中已暗含明确改动指令 → REVISE（例：“任务3风险太高，改成同步调用吧”、“把存储从Redis换成文件”）
            - “整体思路不行，换个方向重来”判定为 REVISE；“整体方案不行，别做了”判定为 REJECT
            - 同时含确认词与修改要求时（如“确认，但任务2改成xx”）判定为 REVISE

            revise 填写规则（仅 REVISE 时必须填，CONFIRM/CLARIFY/REJECT 时省略）：
